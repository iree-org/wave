...
def test_cdna4_mfma_f16(shape: tuple[int], mfma_variant: MMAType):
    # Input sizes
    M = tkl.sym.M
    N = tkl.sym.N
    K = tkl.sym.K
...
def test_cdna4_mfma_bf16(shape: tuple[int], mfma_variant: MMAType):
    # Input sizes
    M = tkl.sym.M
    N = tkl.sym.N
    K = tkl.sym.K
    ...
    def gemm(
        ...
        b: tkl.Memory[K, N, ADDRESS_SPACE, tkl.bf16],
        c: tkl.Memory[M, N, GLOBAL_ADDRESS_SPACE, tkl.f32],
    ):
        c_reg = tkl.Register[M, N, tkl.f32](0.0)
    ...
    a = device_randn(shape[0], shape[2], device="cuda", dtype=torch.bfloat16)
    b = device_randn(shape[2], shape[1], device="cuda", dtype=torch.bfloat16)
    c = device_randn(shape[0], shape[1], device="cuda", dtype=torch.float32)
    asm = gemm(a, b, c)
