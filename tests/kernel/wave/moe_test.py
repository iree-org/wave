...
def nit_torch_ref_moe(a, w1, w2, score, topk):
    m, k = a.shape
    a = a.view(m, -1, k).repeat(1, topk, 1).reshape(-1, k)
    out = torch.zeros(m * topk, w1.shape[1], dtype=a.dtype, device=a.device)
    score = torch.softmax(score, dim=-1, dtype=torch.float32)
    topk_weight, topk_ids = torch.topk(score, topk)
    topk_weight = topk_weight.view(-1)
    topk_ids = topk_ids.view(-1)
...
def nit_tkw(a, w1, w2, score, topk):
    ...
    get_wave_moe_fused_gemm_kernel(m * topk, k, w1.shape[1], w1.shape[0], topk, MMAType.F32_16x16x16_F16, torch.float16)
    return out
