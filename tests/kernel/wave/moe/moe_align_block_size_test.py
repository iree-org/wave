...
def moe_align_block_size_pytorch(
    ...
    sorted_indices = torch.argsort(flat_topk * (num_tokens + 1) + torch.arange(num_tokens, device=device))
    ...
    offsets = torch.cat([torch.zeros(1, device=device), expert_counts.cumsum(0)[:-1]]).long()

    # 3. Calculate local positions (0,1,2,... within each expert)
    local_positions = torch.arange(num_tokens, device=device) - offsets.repeat_interleave(expert_counts)
...
def verify_moe_align_block_size_results(
    ...
    assert torch.all(valid_entries.sort().values == torch.arange(num_tokens, device=device)), "Token IDs corrupted"
    ...
    for expert in range(num_experts):
        ...
        sorted_positions = (sorted_ids[..., None] == original_token_indices).any(-1).nonzero().squeeze(-1)
        ...
        if original_token_indices.numel() > 0:
            ...
            assert torch.all(sorted_ids[expert_start:expert_end] != padding_value), "Padding in middle of expert block"

            # Verify the expert_ids assignments match
            block_start = expert_start // block_size
            block_end = (expert_end + block_size - 1) // block_size
            assert torch.all(expert_ids[block_start:block_end] == expert), "Expert ID mismatch"
    ...
    for expert in range(num_experts):
        ...
        if expert_mask.any():
            ...
            expert_token_mask = (sorted_ids[..., None] == expert_mask.nonzero().squeeze(-1)).any(-1)
...
def test_moe_align_block_size(
    ...
    moe_align_block_size_pytorch(topk_ids, num_experts, block_size, sorted_ids, expert_ids, num_tokens_post_pad)

    verify_moe_align_block_size_results(topk_ids, sorted_ids, expert_ids, num_tokens_post_pad, block_size, num_experts)
