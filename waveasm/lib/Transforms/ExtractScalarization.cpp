...
static Value scalarizeAtIndex(Value v, int64_t k, OpBuilder &rewriter,
  ...
  if (auto constOp = dyn_cast<arith::ConstantOp>(def)) {
    ...
    return arith::ConstantOp::create(rewriter, loc,
                                     rewriter.getIntegerAttr(scalarType, *elemVal));
  }
...
private:
  static bool isDeadVectorOp(Operation *op) {
    ...
    if (op->getNumResults() == 1 &&
        ...
        isa<arith::AddIOp, arith::IndexCastOp, arith::SelectOp,
            arith::CmpIOp, arith::AndIOp, arith::MulIOp, arith::SubIOp,
            arith::TruncFOp>(op))
      return true;
