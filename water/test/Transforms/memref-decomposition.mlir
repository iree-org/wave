...
func.func @load_2d(%arg0: memref<10x20xf32>, %i: index, %j: index) -> f32 {
  ...
  // CHECK: %[[CAST0:.*]] = builtin.unrealized_conversion_cast %[[ARG0]] : memref<10x20xf32> to memref<f32>
  // CHECK: %[[CAST1:.*]] = builtin.unrealized_conversion_cast %[[CAST0]] : memref<f32> to memref<?xi8>
  // CHECK: %[[VIEW:.*]] = memref.view %[[CAST1]][%[[C0]]][] : memref<?xi8> to memref<f32>
  // CHECK: %[[IDX:.*]] = affine.apply
  // CHECK: %[[CAST2:.*]] = builtin.unrealized_conversion_cast %[[VIEW]] : memref<f32> to memref<?xi8>
...
func.func @load_3d(%arg0: memref<4x8x16xf16>, %i: index, %j: index, %k: index) -> f16 {
  // CHECK: %[[C0:.*]] = arith.constant 0 : index
  // CHECK: %[[CAST0:.*]] = builtin.unrealized_conversion_cast %[[ARG0]] : memref<4x8x16xf16> to memref<f16>
  // CHECK: %[[CAST1:.*]] = builtin.unrealized_conversion_cast %[[CAST0]] : memref<f16> to memref<?xi8>
  // CHECK: %[[VIEW:.*]] = memref.view %[[CAST1]][%[[C0]]][] : memref<?xi8> to memref<f16>
  // CHECK: %[[IDX:.*]] = affine.apply
  // CHECK: %[[CAST2:.*]] = builtin.unrealized_conversion_cast %[[VIEW]] : memref<f16> to memref<?xi8>
...
func.func @vector_load_2d(%arg0: memref<10x20xf32>, %i: index, %j: index) -> vector<4xf32> {
  ...
  // CHECK: %[[VIEW:.*]] = memref.view %[[CAST1]][%[[C0]]][] : memref<?xi8> to memref<f32>
  // CHECK: %[[IDX:.*]] = affine.apply
  // CHECK: %[[CAST2:.*]] = builtin.unrealized_conversion_cast %[[VIEW]] : memref<f32> to memref<vector<4xf32>>
  // CHECK: %[[CAST3:.*]] = builtin.unrealized_conversion_cast %[[CAST2]] : memref<vector<4xf32>> to memref<?xi8>
  // CHECK: %[[VIEW2:.*]] = memref.view %[[CAST3]][%[[IDX]]][] : memref<?xi8> to memref<vector<4xf32>>
...
func.func @vector_load_3d(%arg0: memref<4x8x16xf16>, %i: index, %j: index, %k: index) -> vector<8xf16> {
  ...
  // CHECK: %[[VIEW:.*]] = memref.view %[[CAST1]][%[[C0]]][] : memref<?xi8> to memref<f16>
  // CHECK: %[[IDX:.*]] = affine.apply
  // CHECK: %[[CAST2:.*]] = builtin.unrealized_conversion_cast %[[VIEW]] : memref<f16> to memref<vector<8xf16>>
  // CHECK: %[[CAST3:.*]] = builtin.unrealized_conversion_cast %[[CAST2]] : memref<vector<8xf16>> to memref<?xi8>
  // CHECK: %[[VIEW2:.*]] = memref.view %[[CAST3]][%[[IDX]]][] : memref<?xi8> to memref<vector<8xf16>>
...
func.func @multiple_loads(%arg0: memref<10x20xf32>, %i: index, %j: index) -> f32 {
  ...
  // CHECK: %[[CAST2:.*]] = builtin.unrealized_conversion_cast %[[VIEW]] : memref<f32> to memref<?xi8>
  // CHECK: %[[VIEW2:.*]] = memref.view %[[CAST2]][%[[IDX0]]][] : memref<?xi8> to memref<f32>
  // CHECK: %[[LOAD0:.*]] = memref.load %[[VIEW2]][] : memref<f32>
  %0 = memref.load %arg0[%i, %j] : memref<10x20xf32>

  // CHECK: %[[IDX1:.*]] = affine.apply
  // CHECK: %[[CAST3:.*]] = builtin.unrealized_conversion_cast %[[VIEW]] : memref<f32> to memref<?xi8>
  // CHECK: %[[VIEW3:.*]] = memref.view %[[CAST3]][%[[IDX1]]][] : memref<?xi8> to memref<f32>
...
func.func @different_types(%arg0: memref<8x16xi32>, %arg1: memref<4x8xf64>, %i: index, %j: index) -> (i32, f64) {
  ...
  // CHECK: %[[CAST1_0:.*]] = builtin.unrealized_conversion_cast %[[ARG1]] : memref<4x8xf64> to memref<f64>
  // CHECK: %[[CAST1_1:.*]] = builtin.unrealized_conversion_cast %[[CAST1_0]] : memref<f64> to memref<?xi8>
  // CHECK: %[[VIEW1:.*]] = memref.view %[[CAST1_1]][%[[C0]]][] : memref<?xi8> to memref<f64>
  // Then arg0
  // CHECK: %[[CAST0_0:.*]] = builtin.unrealized_conversion_cast %[[ARG0]] : memref<8x16xi32> to memref<i32>
  // CHECK: %[[CAST0_1:.*]] = builtin.unrealized_conversion_cast %[[CAST0_0]] : memref<i32> to memref<?xi8>
  // CHECK: %[[VIEW0:.*]] = memref.view %[[CAST0_1]][%[[C0]]][] : memref<?xi8> to memref<i32>
  // CHECK: %[[IDX0:.*]] = affine.apply
  // CHECK: %[[CAST0_2:.*]] = builtin.unrealized_conversion_cast %[[VIEW0]] : memref<i32> to memref<?xi8>
  ...
  // CHECK: %[[IDX1:.*]] = affine.apply
  // CHECK: %[[CAST1_2:.*]] = builtin.unrealized_conversion_cast %[[VIEW1]] : memref<f64> to memref<?xi8>
...
func.func @reinterpret_cast_0d(%base: memref<f32>) -> memref<?x?xf32, strided<[?, ?], offset: ?>> {
  ...
  // CHECK: %[[VIEW:.*]] = memref.view %[[CAST]][%[[C0]]][] : memref<?xi8> to memref<f32>
  // CHECK: %[[REINTERPRET:.*]] = memref.reinterpret_cast %[[VIEW]] to offset: [0], sizes: [1, 2], strides: [3, 4] : memref<f32> to memref<1x2xf32, strided<[3, 4]>>
...
func.func @reinterpret_cast(%base: memref<100xf32>, %offset: index, %size0: index, %size1: index, %stride0: index, %stride1: index) -> memref<?x?xf32, strided<[?, ?], offset: ?>> {
  // CHECK: %[[C0:.*]] = arith.constant 0 : index
  // CHECK: %[[CAST0:.*]] = builtin.unrealized_conversion_cast %[[BASE]] : memref<100xf32> to memref<f32>
  // CHECK: %[[CAST1:.*]] = builtin.unrealized_conversion_cast %[[CAST0]] : memref<f32> to memref<?xi8>
  // CHECK: %[[VIEW:.*]] = memref.view %[[CAST1]][%[[C0]]][] : memref<?xi8> to memref<f32>
  // CHECK: %[[OFF:.*]] = affine.apply
  // CHECK: %[[CAST2:.*]] = builtin.unrealized_conversion_cast %[[VIEW]] : memref<f32> to memref<?xi8>
  // CHECK: %[[VIEW2:.*]] = memref.view %[[CAST2]][%[[OFF]]][] : memref<?xi8> to memref<f32>
  // CHECK: %[[REINTERPRET:.*]] = memref.reinterpret_cast %[[VIEW2]] to offset: [0], sizes: [%[[SIZE0]], %[[SIZE1]]], strides: [%[[STRIDE0]], %[[STRIDE1]]] : memref<f32> to memref<?x?xf32, strided<[?, ?], offset: ?>>
...
func.func @fat_raw_buffer_cast(%arg0: memref<10x20xf32>) -> memref<10x20xf32, #amdgpu.address_space<fat_raw_buffer>> {
  // CHECK: %[[C0:.*]] = arith.constant 0 : index
  // CHECK: %[[CAST0:.*]] = builtin.unrealized_conversion_cast %[[ARG]] : memref<10x20xf32> to memref<f32>
  // CHECK: %[[CAST1:.*]] = builtin.unrealized_conversion_cast %[[CAST0]] : memref<f32> to memref<?xi8>
  // CHECK: %[[VIEW:.*]] = memref.view %[[CAST1]][%[[C0]]][] : memref<?xi8> to memref<f32>
  // CHECK: %[[FAT:.*]] = amdgpu.fat_raw_buffer_cast %[[VIEW]] : memref<f32> to memref<f32, #amdgpu.address_space<fat_raw_buffer>>
  // CHECK: %[[REINTERPRET:.*]] = memref.reinterpret_cast %[[FAT]] to offset: [0], sizes: [10, 20], strides: [20, 1] : memref<f32, #amdgpu.address_space<fat_raw_buffer>> to memref<10x20xf32, #amdgpu.address_space<fat_raw_buffer>>
  // CHECK: return %[[REINTERPRET]]
