...
// Bitcast forward: i8 -> f4 propagates leading dims, keeps result's last dim.
normalform.module [#wave.normal_form<full_func_boundary>] {
  // CHECK-LABEL: @bitcast_forward_i8_to_f4
  func.func @bitcast_forward_i8_to_f4(%v: !wave.tensor<[@M, @N] of i8, <register>>) {
...
// Bitcast forward with same bitwidth propagates all dimensions unchanged.
normalform.module [#wave.normal_form<full_func_boundary>] {
  // CHECK-LABEL: @bitcast_forward_same_bitwidth
  func.func @bitcast_forward_same_bitwidth(%v: !wave.tensor<[@M, @N] of f8E4M3FN, <register>>) {
    // CHECK: wave.bitcast %{{.*}} : !wave.tensor<[@M, @N] of f8E4M3FN, <register>> to !wave.tensor<[@M, @N] of i8, <register>>
    wave.bitcast %v : !wave.tensor<[@M, @N] of f8E4M3FN, <register>> to !wave.tensor<any of i8, <register>>
    return
  }
}
...
// Bitcast backward with same bitwidth propagates all dimensions unchanged.
normalform.module [#wave.normal_form<full_func_boundary>] {
  // CHECK-LABEL: @bitcast_backward_same_bitwidth
  func.func @bitcast_backward_same_bitwidth() {
    %v = water_test.wave_tensor : !wave.tensor<any of f8E4M3FN, <register>>
    // CHECK: wave.bitcast %{{.*}} : !wave.tensor<[@M, @N] of f8E4M3FN, <register>> to !wave.tensor<[@M, @N] of i8, <register>>
    wave.bitcast %v : !wave.tensor<any of f8E4M3FN, <register>> to !wave.tensor<[@M, @N] of i8, <register>>
    return
  }
}
...
// Bitcast forward with hyperparameters: validates last dimension ratio.
normalform.module [#wave.normal_form<full_func_boundary>] {
  // CHECK-LABEL: @bitcast_forward_hyper_valid
  func.func @bitcast_forward_hyper_valid(%v: !wave.tensor<[@M, @N] of i8, <register>>)
...
// Bitcast with hyperparameters: last dim ratio mismatch.
normalform.module [#wave.normal_form<full_func_boundary>] {
  func.func @bitcast_hyper_invalid(%v: !wave.tensor<[@M, @N] of i8, <register>>)
    ...
    // expected-error @below {{bitcast trailing dimension mismatch}}
    wave.bitcast %v : !wave.tensor<[@M, @N] of i8, <register>> to !wave.tensor<[@M, @K] of f4E2M1FN, <register>>
    return
  }
}
...
// Bitcast with same trailing dim but different element bitwidths is invalid.
normalform.module [#wave.normal_form<full_func_boundary>] {
  func.func @bitcast_wrong_trailing_dim(%v: !wave.tensor<[@M, @N] of i8, <register>>) {
    // expected-error @below {{trailing dimension must be scaled by the element bitwidth ratio (8-bit to 4-bit)}}
