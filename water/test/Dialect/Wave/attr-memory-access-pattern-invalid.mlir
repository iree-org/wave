...
func.func @memory_access_pattern_partial_lds_specification(%value: !wave.tensor<any of f32, <register>>, %mem: !wave.tensor<[@M] of f32, <global>>) {
  wave.write %value, %mem {
    // expected-error @+1 {{when LDS promotion is enabled, all LDS parameters must be specified: lds_block_global_base, lds_block_shape, lds_load_indices, lds_load_vector_sizes, global_store_indices, global_store_vector_sizes}}
    memory_access_pattern = #wave.memory_access_pattern<
...
// Test: Mismatched ranks between global_store_indices and global_store_vector_sizes should fail
func.func @memory_access_pattern_mismatched_global_store_ranks(%value: !wave.tensor<any of f32, <register>>, %mem: !wave.tensor<[@M] of f32, <global>>) {
  wave.write %value, %mem {
    // expected-error @+1 {{global_store_indices rank (1) must match global_store_vector_sizes rank (2)}}
    memory_access_pattern = #wave.memory_access_pattern<
      use_lds_promotion = true,
      group_id = "test",
      lds_block_global_base = #wave.expr_list<[#wave.index_symbol<WG0>] -> (WG0)>,
      ...
      lds_load_vector_sizes = #wave.expr_list<[] -> (64)>,
      global_store_indices = #wave.expr_list<[#wave.index_symbol<T0>] -> (T0)>,
      global_store_vector_sizes = #wave.expr_list<[#wave.symbol<"VEC_M">, #wave.symbol<"VEC_N">] -> (VEC_M, VEC_N)>
    >
  } : !wave.tensor<any of f32, <register>>, !wave.tensor<[@M] of f32, <global>>
  return
}
