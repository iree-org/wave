...
static LogicalResult materializeRegCopy(IRRewriter &rewriter, Operation *op) {
...
static void hoistAllocasFromLoop(IRRewriter &rewriter, scf::ForOp loop) {
  ...
  for (auto [idx, yieldedValue, iterArg, init, result] : llvm::enumerate(
           *yieldedValues, loop.getRegionIterArgs(), loopInits, *loopResults)) {
...
public:
  void runOnOperation() override {
    ...
    getOperation()->walk(
        [&](scf::ForOp forOp) { hoistAllocasFromLoop(rewriter, forOp); });
  }
};
