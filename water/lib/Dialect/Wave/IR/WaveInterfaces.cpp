...
wave::IndexExprsLatticeStorage::IndexExprsLatticeStorage(
    mlir::DictionaryAttr concreteValue, int32_t priority)
...
wave::IndexExprsLatticeStorage wave::IndexExprsLatticeStorage::join(
  ...
  if (lhs.isBottom()) {
    ...
    return IndexExprsLatticeStorage(
        mlir::DictionaryAttr::get(rhs.getConcreteValue().getContext(),
                                  filtered),
        rhs.getPriority());
  }
  ...
  return IndexExprsLatticeStorage(
      mlir::DictionaryAttr::get(
          ctx, llvm::map_to_vector(result,
                                   [](auto &&pair) {
                                     return mlir::NamedAttribute(pair.first,
                                                                 pair.second);
                                   })),
