...
from ...ops.wave_ops import Allocate, IterArg, Iterate, SharedMemoryBarrier, Conditional, get_custom
...
def minimize_shared_allocs(trace: CapturedTrace, minimize_shared_allocs: bool):
    ...
    has_conditionals = any(isinstance(get_custom(node), Conditional) for node in root_graph.nodes)
    if has_conditionals:
