...
LogicalResult handleArithTruncF(Operation *op, TranslationContext &ctx) {
  ...
  if (numElems > 1) {
    // For vector truncf (e.g., vector<4xf32> → vector<4xbf16>):
    // Map the output to the same registers as the input. The actual
    // per-element conversion is deferred to the point of use (the
    // memref.atomic_rmw handler emits V_CVT_BF16_F32 before the atomic).
    // This avoids PackOp/ExtractOp register allocation issues where
    // intermediate results lose their register bindings.
    ctx.getMapper().mapValue(truncOp.getResult(), *src);
  } else {
    auto vregType = ctx.createVRegType();
    Value result;
