...
class VLshlAddU32(ArithmeticInstruction):
    """Fused left shift and add: dst = (src0 << src1) + src2.

    This is a VOP3 instruction that avoids using separate v_add_u32,
    which helps avoid timing issues on gfx950.
    """

    def __init__(
        self,
        destination_register: int,
        src0: str,  # Value to shift (vN or sN)
        src1: int,  # Shift amount (immediate)
        src2: str,  # Value to add (vN or sN)
        comment: str = None,
    ):
        super().__init__(
            "v_lshl_add_u32",
            [f"v{destination_register}", src0, str(src1), src2],
            comment,
        )
