...
class OperationHandlers:
    ...
    def handle_arith_index_cast_op(
        ...
        """Handle arith.index_cast operations - propagate numeric values through cast.

        Propagates integers and SymPy expressions, but NOT strings.
        """
        result_ssa = str(operation.result)
        src_ssa = str(operation.operands[0])

        # Propagate numeric values (int or SymPy expr)
        src_val = kernel_info.index_env.get(src_ssa)
        if isinstance(src_val, (int, sympy.Expr)):
