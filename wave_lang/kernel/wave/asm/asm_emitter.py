...
class AsmEmitter:
    ...
    **Future Work**: Implement offset splitting by adjusting SRD base pointer
    ...
    def _setup_workitem_id_vgprs(self, wg_size: tuple) -> int:
        ...
        wg_size_y = wg_size[1] if len(wg_size) > 1 else 1
        wg_size_z = wg_size[2] if len(wg_size) > 2 else 1
    ...
    def emit_epilogue(
        ...
        wg_size_x, wg_size_y, wg_size_z = (
            wg_size
            if len(wg_size) == 3
            else (wg_size[0], wg_size[1] if len(wg_size) > 1 else 1, 1)
        )
