...
class G2SHandler:
    ...
    def _compute_vaddr(self, src_index_ssa: str, kernel_info: "KernelInfo") -> int:
        ...
        emitter = self.walker.emitter
        src_offset_value = kernel_info.index_env.get(src_index_ssa)

        if isinstance(src_offset_value, int):
            voffset = emitter.vgpr_allocator.alloc_v()
            emitter.emit_instruction(
                VMovB32(
                    voffset, src_offset_value * _F16_ELEM_BYTES, "global offset (bytes)"
                )
            )
            return voffset
