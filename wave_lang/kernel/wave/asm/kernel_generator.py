...
class KernelGenerator:
    ...
    def _handle_g2s_srd_copy(self, instr: KInstr) -> List[str]:
        ...
        lines.append(self._formatter.format("s_mov_b32", 
            defs=[f"s{new_start}"], uses=[f"s{orig_start}"], comment="SRD word0"))
        
        # Word1: base address high (with optional cache swizzle)
        lines.append(self._formatter.format("s_and_b32",
            defs=[f"s{new_start + 1}"], uses=[f"s{orig_start + 1}", "0xffff"]))
        if cache_swizzle != 0:
            lines.append(self._formatter.format("s_or_b32",
                defs=[f"s{new_start + 1}"], uses=[f"s{new_start + 1}", hex(cache_swizzle)],
                comment="cache swizzle"))
        
        # Word2: max buffer size
        lines.append(self._formatter.format("s_mov_b32",
            defs=[f"s{new_start + 2}"], uses=["0x7ffffffd"], comment="SRD word2"))
        
        # Word3: LDS format
        lines.append(self._formatter.format("s_mov_b32",
            defs=[f"s{new_start + 3}"], uses=["0x27000"], comment="SRD word3"))
    ...
    def _handle_srd_load_base(self, instr: KInstr) -> str:
        ...
        return self._formatter.format("s_load_dwordx2",
            defs=[f"s[{base}:{base+1}]"], uses=[kernarg_phys, offset], comment=instr.comment)
    ...
    def _handle_srd_fill_size(self, instr: KInstr) -> str:
        ...
        return self._formatter.format("s_mov_b32",
            defs=[f"s{base + 2}"], uses=[hex(size_val)], comment=instr.comment)
    ...
    def _handle_srd_fill_stride(self, instr: KInstr) -> str:
        ...
        return self._formatter.format("s_mov_b32",
            defs=[f"s{base + 3}"], uses=[hex(stride_val)], comment=instr.comment)
    ...
    def _handle_compare(self, instr: KInstr) -> str:
        ...
        if len(instr.uses) >= 2:
            ...
            return self._formatter.format("s_cmp_lt_u32", uses=[op0, op1], comment=instr.comment)
        return None
    ...
    def _handle_loop_inc(self, instr: KInstr) -> str:
        ...
        if len(instr.uses) >= 2:
            ...
            return self._formatter.format("s_add_u32", 
                defs=[counter], uses=[counter, step], comment=instr.comment)
        return None
    ...
    def _handle_mfma_acc(self, instr: KInstr) -> str:
        ...
        if len(instr.uses) >= 3:
            ...
            return self._formatter.format("v_mfma_f32_16x16x16_f16",
                defs=[acc], uses=[a, b, acc], comment=instr.comment)
        return None
    ...
    def _handle_buffer_load_lds(self, instr: KInstr) -> str:
        ...
        if len(instr.uses) >= 3:
            ...
            srd = self._resolve_reg_range(instr.uses[1]) if hasattr(instr.uses[1], 'base_reg') else self._resolve_operand(instr.uses[1])
            soffset = self._resolve_operand(instr.uses[2])
            
            # Determine mnemonic based on instruction name
            mnemonic = "buffer_load_dword" if "dword_lds" in instr.name else "buffer_load_dwordx4"
            
            return self._formatter.format(mnemonic + "_lds",
                uses=[vaddr, srd, soffset], comment=instr.comment)
        return None
    ...
    def _handle_init_acc_quad(self, instr: KInstr) -> List[str]:
        ...
        if len(instr.defs) >= 1:
            ...
            for i in range(4):
                ...
                lines.append(self._formatter.format("v_mov_b32",
                    defs=[f"v{base_idx + i}"], uses=[0], comment=comment))
            return lines
        return None
