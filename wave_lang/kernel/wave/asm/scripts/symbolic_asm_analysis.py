...
class SymbolicAsmAnalyzer:
    ...
    def process_instruction(self, line: str) -> bool:
        ...
        # Skip directives
        if op.startswith('.') or op.startswith('s_waitcnt') or op.startswith('s_barrier'):
            return False
        ...
        if op in ('v_and_b32', 'v_and_b32_e32'):
            ...
            if len(args) >= 3:
                dst, src0, src1 = args[0], args[1], args[2]
                self.set_reg_value(dst, self.parse_operand(src0) + self.parse_operand(src1))
                return True
            ...
            if len(args) >= 3:
                ...
                self.set_reg_value(dst, Symbol(dst.replace('[', '_').replace(']', '_').replace(':', '_')))
                return True
                ...
                if vaddr_val:
                    ...
                    for tid_x, tid_y, desc in samples:
                        try:
                            ...
                            try:
                                result = int(result)
                                print(f"  {desc}: tid_x={tid_x}, tid_y={tid_y} -> vaddr={result}")
                            except:
                                print(f"  {desc}: tid_x={tid_x}, tid_y={tid_y} -> vaddr={result}")
