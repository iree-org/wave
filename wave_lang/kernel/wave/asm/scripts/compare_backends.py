...
BENCHMARK_CONFIGS_PATH = Path(__file__).parent.parent.parent / "perf/benchmark_configs.json"
...
def load_benchmark_config(name: str) -> dict:
    ...
    if not BENCHMARK_CONFIGS_PATH.exists():
        raise FileNotFoundError(f"Benchmark config file not found: {BENCHMARK_CONFIGS_PATH}")
...
def create_gemm_kernel(config: dict):
    ...
    return gemm_kernel, (M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, ADDRESS_SPACE, ADDRESS_SPACE_0)
...
def compile_backend(
    ...
    options = WaveCompileOptions(
        subs={
            ...
            ADDRESS_SPACE: SHARED_ADDRESS_SPACE, ADDRESS_SPACE_0: GLOBAL_ADDRESS_SPACE
        },
        canonicalize=True,
        backend=backend,
        wave_runtime=True,
...
def classify_instruction(line: str) -> str:
    ...
    if not line_lower or line_lower.startswith('.') or line_lower.startswith(';'):
        return "skip"
    if ':' in line_lower and not any(op in line_lower for op in ['v_', 's_', 'ds_', 'buffer_', 'global_']):
        return "skip"  # Label
    ...
    # Branch/control
    if any(x in line_lower for x in ['s_cbranch', 's_branch', 's_setpc', 's_call', 's_endpgm']):
        return "branch"
    
    # LDS operations
    if line_lower.startswith('ds_') or 'ds_read' in line_lower or 'ds_write' in line_lower:
        return "ds"
    
    # Vector memory
    if any(x in line_lower for x in ['buffer_load', 'buffer_store', 'global_load', 'global_store', 'flat_load', 'flat_store']):
        return "vmem"
...
def format_metrics_comparison(llvm_metrics: InstructionMetrics, asm_metrics: InstructionMetrics) -> str:
    """Format a comparison table of metrics."""
    lines = []
    lines.append("=" * 70)
    lines.append("Instruction Metrics Comparison")
    lines.append("=" * 70)
    lines.append(f"{'Category':<25} {'LLVM':>10} {'ASM':>10} {'Diff':>10} {'Ratio':>10}")
    lines.append("-" * 70)
    ...
    lines.append(row("  buffer_load...lds", llvm_metrics.buffer_load_lds, asm_metrics.buffer_load_lds))
    ...
    lines.append(row("  M0 setup (s_mov m0)", llvm_metrics.m0_setup, asm_metrics.m0_setup))
    lines.append(row("  v_readfirstlane", llvm_metrics.readfirstlane, asm_metrics.readfirstlane))
    lines.append("=" * 70)
    
    return "\n".join(lines)


def format_resource_comparison(llvm_result: BackendResult, asm_result: BackendResult) -> str:
...
def find_context_around_pattern(asm_text: str, pattern: str, context_before: int = 10, context_after: int = 3, max_occurrences: int = 3) -> str:
...
def generate_report(
    ...
    lines.append(f"  Blocks: BLOCK_M={config['block_m']}, BLOCK_N={config['block_n']}, BLOCK_K={config['block_k']}")
    ...
    overhead_ratio = asm_metrics.total / llvm_metrics.total if llvm_metrics.total > 0 else 0
    ...
    lines.append(f"Instruction overhead: {overhead_ratio:.2f}x ({asm_metrics.total - llvm_metrics.total:+d} instructions)")
    lines.append("")
    
    # Identify likely causes
    causes = []
    if extra_waits > 0:
        causes.append(f"- Extra waits: +{extra_waits} s_waitcnt (conservative scheduling?)")
    if extra_barriers > 0:
        causes.append(f"- Extra barriers: +{extra_barriers} s_barrier")
    if extra_nops > 0:
        causes.append(f"- Extra NOPs: +{extra_nops} s_nop (hazard mitigation on gfx950?)")
    if extra_salu > 10:
        causes.append(f"- Extra SALU: +{extra_salu} (addressing overhead / missed CSE?)")
    if extra_valu > 10:
        causes.append(f"- Extra VALU: +{extra_valu} (index computation overhead?)")
    if extra_m0 > 0:
        causes.append(f"- Extra M0 setup: +{extra_m0} (per-operation M0 vs. precomputed?)")
    if extra_readfirstlane > 0:
        causes.append(f"- Extra readfirstlane: +{extra_readfirstlane} (scalar extraction overhead?)")
    if extra_vgpr > 0:
...
""",
    ...
    parser.add_argument('--passing', action='store_true', help='Use passing preset config')
    parser.add_argument('--failing', action='store_true', help='Use failing preset config (default)')
    parser.add_argument('--benchmark', '-b', type=str, help='Load config from benchmark_configs.json by name')
    parser.add_argument('--output-dir', '-o', type=str, default='.', help='Output directory for artifacts')
    parser.add_argument('--no-g2s', action='store_true', help='Disable global_to_shared for comparison')
    parser.add_argument('--show-context', action='store_true', help='Show context around key patterns')
    ...
    print(f"Blocks: BLOCK_M={config['block_m']}, BLOCK_N={config['block_n']}, BLOCK_K={config['block_k']}")
    ...
    llvm_result = compile_backend(
        kernel_llvm, symbols_llvm, config, "llvm",
        use_global_to_shared=use_global_to_shared,
    ...
    print(f"  Resources: VGPR={llvm_result.vgpr_count}, SGPR={llvm_result.sgpr_count}, LDS={llvm_result.lds_size}")
    
    # Compile ASM backend
    print("\n>>> Compiling ASM backend...")
    kernel_asm, symbols_asm = create_gemm_kernel(config)
    asm_result = compile_backend(
        kernel_asm, symbols_asm, config, "asm",
        use_global_to_shared=use_global_to_shared,
    ...
    print(f"  Resources: VGPR={asm_result.vgpr_count}, SGPR={asm_result.sgpr_count}, LDS={asm_result.lds_size}")
    ...
    report = generate_report(
        config, config_name, llvm_result, asm_result,
        llvm_metrics, asm_metrics, use_global_to_shared,
    )
    ...
    if args.show_context:
        ...
        print("=" * 80)
        print(find_context_around_pattern(llvm_text, "buffer_load", context_before=15, max_occurrences=2))
        
        print("\n" + "=" * 80)
        print("Context: ASM around first buffer_load...lds")
        print("=" * 80)
        print(find_context_around_pattern(asm_text, "buffer_load", context_before=15, max_occurrences=2))
