...
def get_gemm_unroll_then_pipeline_kernel_and_schedule(
    shape: tuple[int, int, int] = (256, 256, 512),
    mfma_variant: tkw.MMAType = tkw.MMAType.F32_16x16x16_F16,
    unroll_factor: int = 2,
    compile_to_mlir: bool = True,
    ...
    """
    Returns a GEMM kernel with unroll-then-pipeline schedule.

    Demonstrates: unroll first, then pipeline the unrolled iterate.
    ...
    def unroll_then_pipeline_schedule():
        ...
        # Get nodes for pipelining (tags are propagated to unrolled copies)
        load_a = tkw.get_node_by_tag_and_type("read_a", tkw.Read)
        global_load_a, shared_load_a = tkw.partition_by_address_space(
            load_a, GLOBAL_ADDRESS_SPACE
        )
    ...
    return gemm_unroll_then_pipeline, unroll_then_pipeline_schedule, options
