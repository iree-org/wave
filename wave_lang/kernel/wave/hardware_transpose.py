...
def compute_transpose_indices(shape, dtype_width, thread_id):
    """
    ...
    tid_mod_64 = thread_id % WAVE_SIZE

    # Base pattern components
    base_first = tid_mod_group * vw
    base_second = sympy.floor(tid_mod_64 / group_size)
    ...
    if is_wide_tile:
        ...
        second_dim = wide_second_base + sympy.floor(tid_mod_64 / 32) * offset
    elif is_tall_tile:
        # Tall tiles: multiple wave-groups vertically
        first_dim = base_first
        second_dim = base_second + group_size * sympy.floor(tid_mod_64 / TILE_THRESHOLD)
    else:
...
def fetch_delinearized_indices(shape, dtype_width, thread_id):
    ...
    return compute_transpose_indices(shape, dtype_width, thread_id)
